/**
 * The closed loop, driven once against the real Supabase project.
 *
 * This is the chain itself — reset, a Visitor joins, Simulate Rush, the engine's
 * Recommendation, approve, accept, apply, and the assertion that the Visitor's
 * recomputed ETA is STRICTLY LOWER than before. `goldenPath.ts` runs it once and
 * `acceptanceRun.ts` runs it twice with extra assertions; there is deliberately
 * only ONE implementation of the chain, because two would eventually disagree
 ...
 *
 * Every function that reports takes its `Report` as an argument rather than
 * sharing a module-level one, so a second pass gets a clean slate — a run that
 * silently accumulated the first pass's assertions could not prove the second
 * pass stood on its own.
 */
...
import { fetchFacilityRows, rpc, rpcExpectingFailure } from "./client.js";
import { HarnessFailure, Report, minutes } from "./report.js";

/**
 * What one pass through the closed loop produced. The golden path prints the
 * headline and stops; the acceptance run asserts against the rest.
 */
export interface ClosedLoopPass {
  /** The closing block, ready to print. */
  headline: string[];
  /** Examination Cell — the Service the demo is about. */
  serviceId: string;
  visitorTokenId: string;
  visitorTokenNumber: string;
  recommendationId: string;
  interventionId: string;
  estimatedMinutesReturned: number;
  /** Held constant across the pass, which is what makes the drop attributable. */
  customersAhead: number;
  etaBeforeMinutes: number;
  etaAtRushPeakMinutes: number;
  etaAfterMinutes: number;
  activeCountersBefore: number;
  activeCountersAfter: number;
  /** The Tokens Simulate Rush injected into the Service under demonstration. */
  simulatedTokenIds: string[];
  /** The timeline as Postgres wrote it, oldest first. */
  timeline: InterventionEventRow[];
  /** The projection taken after the Intervention was applied. */
  appliedProjection: FacilityProjection;
}

/**
 * The lifecycle the timeline must tell, in this order, every single time. One
 * declaration, shared with the acceptance run — a second copy of the chain is
...
export async function project(client: SupabaseClient): Promise<FacilityProjection> {
  const rows = await fetchFacilityRows(client);
  return projectFacility(rows, { now: Date.now() });
}

export function requireEta(
...
export function assertSeededBaseline(
  report: Report,
  projection: FacilityProjection,
  label: string,
): void {
...
export async function runClosedLoopPass(
  report: Report,
  client: SupabaseClient,
): Promise<ClosedLoopPass> {
  // ---- 0. Preflight -------------------------------------------------------
  ...
  report.info(await resetToBaseline(report, client));

  const baseline = await project(client);
  assertSeededBaseline(
    report,
    baseline,
    "The seeded baseline is in place, so the run is repeatable",
  );
  ...
  if (recommendation === null) throw new HarnessFailure("unreachable");
  report.assert(
  ...
  const timeline = await loadTimeline(report, client, interventionId);
  const actualChain = timeline.map((event) => event.event_type);
  report.assert(
    "The events are in causal order",
    actualChain.join(" -> ") === CAUSAL_CHAIN.join(" -> "),
    actualChain.join(" -> "),
  );
  ...
  const simulatedTokenIds = (rushExamination?.queue ?? [])
  ...
  return {
    ...
    visitorTokenId,
    visitorTokenNumber: tokenNumber,
    recommendationId,
    interventionId,
    estimatedMinutesReturned: recommendation.estimatedMinutesReturned,
    customersAhead: etaBefore.customersAhead,
    etaBeforeMinutes: etaBefore.predictedWaitMinutes,
    etaAtRushPeakMinutes: etaAfterRush.predictedWaitMinutes,
    etaAfterMinutes: etaAfter.predictedWaitMinutes,
    activeCountersBefore: rushExamination?.activeCounters ?? 0,
    activeCountersAfter: appliedExamination?.activeCounters ?? 0,
    simulatedTokenIds,
    timeline,
    appliedProjection,
  };
...
export async function restoreBaseline(
  ...
  report.begin("end", "Leave the database at the seeded baseline");
  report.info(await resetToBaseline(report, client));

  const projection = await project(client);
  assertSeededBaseline(report, projection, "The baseline queue and capacity are back");
